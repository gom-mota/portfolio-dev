...
const Home = ({ router }) => {
    ...
    function validateAPI(url) {
        const regex =
            /^(?:https?:\/\/)?(w{3}gitconnected.com\/v1\/portfolio\/.*\.)?[\w_-]+((\.\w{2,}){1,2})(\/([\w\._-]+\/?)*(\?[\w_-]+=[^\?\/&]*(\&[\w_-]+=[^\?\/&]*)*)?)?$/gm;
    ...
    useEffect(() => {
        if (
            router.query.apiurl != null &&
            validateAPI(router.query.apiurl) == true
        ) {
    ...
    return (
        <Main>
            ...
            <Footer>
                <div>
                    Criado por{" "}
                    <a href="https://github.com/gom-mota/">Gabriel Mota</a> |
                    &copy; 2020
                </div>
            </Footer>
        </Main>
